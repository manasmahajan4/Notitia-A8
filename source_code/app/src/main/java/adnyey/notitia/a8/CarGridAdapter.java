...
public class CarGridAdapter extends StickyHeaderGridAdapter {
    ...
    CarGridAdapter(List<List<GItem>> labels, Context mContext) {
        this.labels = labels;
    ...
    public void onBindItemViewHolder(StickyHeaderGridAdapter.ItemViewHolder viewHolder, final int section, final int position) {
        ...
        myDbHelper = CarDBAccess.getInstance(mContext);
        BasicInfo basic = myDbHelper.getBasic(label.getId());

        try {
            int id = mContext.getResources().getIdentifier(basic.getImg(), "drawable", mContext.getPackageName());
            Glide.with(mContext)
                    .load(id)
                    .apply(options)
                    .thumbnail(0.1f)
